...
interface TrackCheckNodeData {
  label: string;
  text?: string;
  answers?: FlowAnswer[];
}

function TrackCheckNodeComponent({ data }: NodeProps<TrackCheckNodeData>) {
  const config = NODE_TYPE_CONFIG.track_check;
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-yellow-200 px-2 py-1.5 space-y-1">
        {answers.map((ans, i) => (
          <div key={ans.id || i} className="flex items-center justify-between">
            ...
            <Handle
              ...
              id={ans.id || `answer-${i}`}
              className="!w-2.5 !h-2.5 !bg-yellow-500 !border-2 !border-white !relative !transform-none !top-auto !right-auto"
...
export default memo(TrackCheckNodeComponent);
